...
function ScanningScreen({ url, liveData }) {
  ...
  if (liveData.perf != null) {
    ...
    if (liveData.perf.coreWebVitals?.lcp) {
      rows.push({ key: 'lcp', icon: '⏱', text: `LCP: ${liveData.perf.coreWebVitals.lcp} — load speed of your largest element` })
    }
  }
  ...
  if (liveData.benchmarkLabel) {
    rows.push({ key: 'industry', icon: '📊', text: `Industry: ${liveData.benchmarkLabel} — comparing against benchmarks` })
  }
  if (liveData.social) {
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{
            ...
            <div style={{
              height: '100%', background: '#2a5c45', borderRadius: 2,
              width: `${progress}%`,
          ...
          {rows.length > 0 && (
            <div style={{
              ...
              border: '1px solid rgba(28,25,23,0.08)', borderRadius: 14,
              padding: '4px 0',
            }}>
              {rows.map((row, i) => (
                <div key={row.key} className="live-row" style={{
                  animationDelay: `${i * 0.06}s`,
                  display: 'flex', alignItems: 'flex-start', gap: 10,
                  padding: '11px 18px',
                  borderBottom: i < rows.length - 1 ? '1px solid rgba(28,25,23,0.05)' : 'none',
                }}>
          ...
          {rows.length === 0 && (
            <div style={{
              ...
              <p style={{ fontSize: 13, color: '#a09890', fontWeight: 300, animation: 'ringPulse1 1.8s ease infinite' }}>
